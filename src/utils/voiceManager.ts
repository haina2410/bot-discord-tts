...
export class VoiceManager {
     ...
     */
    async joinVoiceChannel(guildId: string, channelId?: string): Promise<VoiceConnection | null> {
        try {
            ...
            if (!channel || !(channel instanceof VoiceChannel)) {
                Logger.error(`❌ Voice channel not found or invalid: ${targetChannelId}`);
                return null;
            }
            ...
            if (!permissions?.has([PermissionsBitField.Flags.Connect, PermissionsBitField.Flags.Speak])) {
                Logger.error('❌ Bot lacks permission to connect or speak in voice channel');
                return null;
            }
            ...
            await entersState(connection, 'Ready', 10000);
     ...
     */
    async playAudioFile(guildId: string, audioFilePath: string): Promise<VoicePlaybackResult> {
        try {
            // Ensure we're connected to a voice channel
            let connection = this.connections.get(guildId);
            if (!connection) {
                connection = await this.joinVoiceChannel(guildId);
                if (!connection) {
                    return {
                        success: false,
    ...
    isConnected(guildId: string): boolean {
        const connection = this.connections.get(guildId);
        return connection !== undefined && connection.state.status !== 'destroyed';
    }
