...
import type { UserProfile, ChannelContext, MessageContext } from './contextManager.js';
...
export class DatabaseContextManager {
    ...
    async buildMessageContext(
        ...
        const userProfile = await this.getUserProfile(message.author.id, message.author.username);
        
        // Get or create channel context
        const channelContext = await this.getChannelContext(message.channel.id, message.channel.name);
     ...
     */
    private async getUserProfile(userId: string, username: string): Promise<UserProfile> {
     ...
     */
    private async getChannelContext(channelId: string, channelName: string): Promise<ChannelContext> {
     ...
     */
    private async updateUserProfile(profile: UserProfile, message: ProcessedMessage): Promise<void> {
     ...
     */
    private async updateChannelContext(context: ChannelContext, message: ProcessedMessage): Promise<void> {
     ...
     */
    async getConversationHistory(channelId: string, limit: number = 50): Promise<Array<{
        role: 'user' | 'assistant';
        content: string;
        timestamp: number;
        author?: string;
    ...
    async cleanup(olderThanDays: number = 30): Promise<void> {
        Logger.info(`🧹 Starting database-backed context cleanup (older than ${olderThanDays} days)...`);
        
        await this.crud.cleanupOldData(olderThanDays);
        
        const stats = await this.getStats();
        Logger.info(`🧹 Context cleanup completed. ${stats.userProfiles} users, ${stats.channelContexts} channels, ${stats.totalMessages} messages`);
    }
     ...
     */
    async getChannelContextById(channelId: string): Promise<ChannelContext | null> {
