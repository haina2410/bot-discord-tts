...
export class ChannelManager {
    ...
    private initializeChannels() {
        ...
        const additionalChannels = process.env.ADDITIONAL_LISTEN_CHANNELS?.split(',');
     ...
     */
    getChannelInfo(channel: Channel): { name: string; type: string; isListening: boolean } {
        const channelName = channel.type === ChannelType.DM ? 'DM' : 
                           'name' in channel ? (channel.name || 'Unnamed') : 'Unknown';
    ...
    logConfiguration() {
        ...
        if (this.listenChannels.size > 0) {
            Logger.info(`🎯 Listening to ${this.listenChannels.size} specific channel(s):`);
            ...
            Logger.info('🌐 Listening to ALL channels (no specific channels configured)');
        }
