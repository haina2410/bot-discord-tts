...
export class DatabaseManager {
  ...
  private logDatabaseInfo(): void {
    ...
    try {
      ...
      Logger.info(
        `   Foreign keys: ${
          this.db.pragma("foreign_keys")[0].foreign_keys
            ? "enabled"
            : "disabled"
        }`
      );
      Logger.info(
        `   Journal mode: ${this.db.pragma("journal_mode")[0].journal_mode}`
      );
    } catch (error) {
  ...
  getStats(): any {
    ...
    try {
      const pageInfo = this.db
        .prepare("SELECT * FROM pragma_page_count(), pragma_page_size()")
        .get() as any;
      const size = pageInfo.page_count * pageInfo.page_size;

      const tableStats = this.db
        .prepare(
          `
                SELECT 
                    name as table_name,
      ...
      return {
        ...
        foreignKeysEnabled:
          this.db.pragma("foreign_keys")[0].foreign_keys === 1,
