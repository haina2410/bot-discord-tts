  ...
  try {
    ...
    switch (operation) {
      case "user":
        ...
        break;
      case "channel":
        result = await testChannelContext(
          crud,
          interaction.channelId,
          interaction.channel?.type?.toString() || "unknown"
        );
        break;
      case "conversation":
        result = await testConversationHistory(
          crud,
          interaction.channelId,
          interaction.user.id
        );
...
async function testUserProfile(
  ...
  try {
    ...
    if (updatedProfile) {
      results.push(
        `   Updated interaction count: ${updatedProfile.interactionCount}`
      );
      results.push(
        `   Updated interests: ${updatedProfile.interests.join(", ")}`
      );
    }
    ...
    if (stats) {
      ...
      results.push(
        `   Interests: ${stats.interests}, Personality Traits: ${stats.personalityTraits}`
      );
    } else {
...
async function testChannelContext(
  crud: DatabaseCRUD,
  channelId: string,
  channelType: string
): Promise<string> {
  ...
  try {
    ...
    results.push(
      created
        ? "✅ Channel context created successfully"
    ...
    results.push(
      updated
        ? "✅ Channel context updated successfully"
        : "❌ Failed to update channel context"
    );
    ...
    if (stats) {
      ...
      results.push(
        `   Topics: ${stats.recentTopics}, Active Users: ${stats.activeUsers}`
      );
    } else {
...
async function testConversationHistory(
  ...
  try {
    ...
    results.push(
      message2Id
        ? "✅ Bot message added successfully"
        : "❌ Failed to add bot message"
    );
    ...
    if (history.length > 0) {
      ...
      for (const msg of history.slice(-2)) {
        ...
        results.push(
          `   [${msg.role}] ${msg.author}: ${msg.content.substring(0, 50)}...`
        );
      }
    } else {
...
async function testAllOperations(
  ...
  const userResult = await testUserProfile(
    crud,
    interaction.user.id,
    interaction.user.username
  );
  ...
  const channelResult = await testChannelContext(
    crud,
    interaction.channelId,
    interaction.channel?.type?.toString() || "unknown"
  );
  ...
  const conversationResult = await testConversationHistory(
    crud,
    interaction.channelId,
    interaction.user.id
  );
